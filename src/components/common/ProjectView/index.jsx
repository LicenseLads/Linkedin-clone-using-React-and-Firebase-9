...
const ProjectView = ({currentUserId}) => {
  ...
  const displayDate = project.updated_at ? formatDate(project.updated_at) : formatDate(project.created_at);
...
{currentUserId === project.author && (
  <button className="edit-button" onClick={() => navigate(`/edit-project/${projectId}`)}>Edit</button>
)}
      </div>
              ...
              {loadingRepo ? (
                ...
                <>
                  <div className="github-card-header">
                    <h3 className="github-card-title">{repoData.full_name}</h3>
                  </div>
                  <p className="github-card-description">{repoData.description}</p>
